...
* **Basic**: What steps does Lattice take when starting up after an ungraceful crash?
  - **Answer**: Lattice initiates startup recovery through a strictly ordered pipeline: (1) Boot Discovery (`DiscoverActiveManifest`): validates the database directory, strictly parses the authoritative `CURRENT` pointer, resolves the canonical `MANIFEST-%06d` path, and securely opens the active manifest regular file using descriptor identity verification (`os.SameFile`) to eliminate TOCTOU attacks; (2) Manifest Replay: reads sequential `VersionEdit` records to reconstruct the active `Version` levels ($L_0..L_6$) and asserts all referenced `.sst` files exist on disk; (3) WAL Discovery & Replay: identifies uncommitted WAL segments newer than the manifest checkpoint and replays valid records into the active MemTable; (4) Orphan Cleanup: purges unreferenced crash-window temporary files.
* **Intermediate**: Why is `CURRENT` treated as strictly authoritative during recovery, and why must the engine never fall back to scanning for the highest MANIFEST number on disk?
  - **Answer**: In an LSM storage engine, compaction or metadata rotation may stage a new manifest file (e.g. `MANIFEST-000005`) that is incompletely written or uncommitted if the process crashes before the atomic `CURRENT` swap (`os.Rename(CURRENT.tmp, CURRENT)` + directory fsync). Scanning the directory and arbitrarily picking the highest manifest number could cause the engine to boot against an uncommitted, torn, or orphaned manifest. If `CURRENT` is missing or corrupted, the system must fail closed rather than guess, preventing silent causal inversion or state divergence.
* **Deep**: Why is pathname validation alone insufficient when opening the active MANIFEST during recovery, and how does Lattice eliminate TOCTOU substitution attacks?
  - **Answer**: Pathname validation (`os.Lstat(path)`) only verifies the filesystem state at the instant of inspection. Between `Lstat` and `os.OpenFile`, an unprivileged concurrent process or attacker could swap the path with a symlink, directory, or different file. Lattice enforces descriptor-centric pinning: it inspects `lstatBefore`, opens the descriptor with `O_RDONLY | O_NOFOLLOW`, inspects the opened descriptor via `file.Stat()` (`fstat`), re-inspects the path post-open (`lstatAfter`), and asserts `os.SameFile(fstat, lstatBefore)` and `os.SameFile(fstat, lstatAfter)`. If any substitution occurred, recovery fails closed immediately and cleans up the descriptor via `defer`.
* **Deep**: How does Lattice manage descriptor ownership during boot discovery?
  - **Answer**: `DiscoverActiveManifest` returns `*DiscoveredManifest` which explicitly assumes exclusive ownership of the opened `*os.File`. If discovery encounters any error during validation, directory checks, or post-open pinning, a deferred cleanup closure guarantees the descriptor is closed immediately. Once successfully returned, the caller owns the descriptor and is responsible for calling `disc.Close()`, which is safe and idempotent.
* **Follow-up**: How do you distinguish between an uncommitted partial write at EOF and data corruption in the middle of a log?
* **Deep**: Why is the MANIFEST designed as an append-only delta log rather than periodically overwriting a snapshot file?
  - **Answer**: LSM-tree state transitions occur frequently via MemTable flushes and compaction runs. Overwriting a full snapshot file on every transition would introduce catastrophic write amplification and synchronous I/O stalls on critical write paths. An append-only delta log of `VersionEdit` records requires only a tiny append (often < 1 KB) plus an `fsync`. Full state is reconstructed deterministically at boot by sequentially replaying the deltas starting from an empty state.
* **Deep**: Why does Lattice validate record framing and CRC32 before decoding a `VersionEdit`, and why must replay fail closed rather than skipping corrupted records?
  - **Answer**: The 32-bit payload length field on disk is completely untrusted. Decoding a `VersionEdit` without framing verification exposes the engine to allocation exhaustion (DoS via huge lengths) or parsing malformed data. Calculating and verifying CRC32-IEEE over `[PayloadLen || Payload]` ensures bit-rot or torn writes are caught before parsing. Replay must fail closed immediately because `VersionEdit` records are sequential state transitions ($V_N = \text{Apply}(E_N, V_{N-1})$). Skipping a corrupted edit would cause causal inversion, zombie SSTables, or loss of deletion markers, leading to silent data corruption.
* **Deep**: Why are historically deleted SSTables permitted to be missing from disk during MANIFEST replay, while missing active SSTables abort with `ErrMissingSSTable`?
  - **Answer**: The MANIFEST preserves the historical timeline of additions and deletions. When a compaction merges SSTables, it records `DeleteFile(level, fileNum)` for the inputs and `AddFile(...)` for the outputs. Obsolete SSTables are subsequently unlinked from disk by background garbage collection. Requiring every file ever mentioned in the MANIFEST to exist on disk would cause recovery to fail on legitimately reclaimed files. Only the *final active* SSTables present in the reconstructed `Version` levels ($L_0..L_6$) represent live data, and every such file must exist as a verified regular file on disk; otherwise, Lattice aborts with `ErrMissingSSTable` to prevent serving corrupted partial datasets.
* **Deep**: How does Lattice prevent state pollution if a MANIFEST replay fails midway through recovery?
  - **Answer**: `ReplayManifest` accumulates all state transitions inside an isolated, private in-memory builder (`versionBuilder`) without publishing or attaching partial versions to any `VersionSet`. All disk inspections are read-only (`os.Lstat`). If an error occurs at record $K$, the function returns `*ReplayError` immediately, and the in-memory builder is discarded. The live `VersionSet`, `CURRENT` pointer, `MANIFEST` file, and SSTables remain 100% untouched.
* **Intermediate**: Why does `ReplayManifest` borrow the pinned file descriptor from `DiscoverActiveManifest` rather than reopening the file path?
  - **Answer**: Reopening the file by pathname would reintroduce TOCTOU race conditions where an attacker or concurrent process could replace the verified MANIFEST file between boot discovery and replay. Reusing the pinned descriptor `discovered.File` guarantees that replay operates on the exact, authenticated inode established during discovery. The descriptor is borrowed rather than closed by `ReplayManifest`, maintaining clear lifecycle ownership with the caller.
* **Deep**: How does Lattice ensure that replaying the WAL after a crash does not result in duplicate keys or sequence number regressions?
  - **Answer**: Lattice enforces cross-subsystem watermark filtering (`P07-S02-M01`). During startup recovery, the MANIFEST is replayed first via `version.ReplayManifest`, reconstructing the durable `Version` and establishing the authoritative sequence number watermark `LastSeqNum`. Next, `wal.RecoverWAL` scans all WAL segments ($1..N$), validates physical framing, verifies CRC32 checksums, and enforces global sequence monotonicity across segments. In the replay sink, each record's sequence number is compared against the durable watermark: records with `SeqNum <= LastSeqNum` are already safely persisted in durable SSTables and are intentionally filtered out (not inserted into the MemTable); only records with `SeqNum > LastSeqNum` are replayed into a fresh active `MemTable`. Furthermore, `nextSeqNum` is initialized to `max(manifestCheckpoint, wal.LastSeqNum)`, establishing the authoritative highest assigned sequence number watermark. When subsequent writes arrive, they allocate via `nextSeqNum.Add(1)`, guaranteeing strictly increasing sequence numbers without gaps or regression.
* **Follow-up**: What happens if the machine crashes while the recovery process itself is running?
  - **Answer**: Recovery in Lattice is designed with strict idempotency and zero non-atomic side-effects. Neither `ReplayManifest` nor `RecoverWAL` mutates the active `CURRENT` pointer, `MANIFEST` log, or immutable `.sst` files. All recovery state is built in private in-memory staging structures (`versionBuilder`, private `memtable.NewSkipList()`). The only disk modification permitted during WAL recovery is the physical truncation of an incomplete torn tail at the very EOF of the latest segment (`wal.RecoverSegment`), which is itself idempotent (truncating a truncated file to the same boundary is a no-op). If a crash interrupts recovery at any point, the on-disk state remains clean and uncorrupted, and subsequent reboot attempts will repeat discovery and replay with identical, deterministic outcomes.
* **"Did You Actually Build This?"**: If a flush completes and writes an SSTable to disk, but the process is killed before the WAL is deleted, how do you prevent the recovered database from inserting duplicate records from the old WAL?
  - **Answer**: This is the classic dual-write crash window in LSM-tree engines: (1) MemTable flushes to SSTable `000004.sst`; (2) MANIFEST records `AddFile` and updates `LastSeqNum = 500`; (3) Crash occurs before WAL segment 1 (containing seq 1..500) can be unlinked. At reboot, recovery reads the MANIFEST and learns that `LastSeqNum = 500`. When `wal.RecoverWAL` reads segment 1, it physically decodes and CRC-verifies all 500 records to prove log integrity, but the engine replay sink observes `rec.SeqNum <= 500` for every record and deliberately skips insertion into the active MemTable. Only records with `seq > 500` (e.g. 501..550 written to WAL after the flush began) are inserted. Zero duplicate records are inserted into the MemTable, preventing duplicate version entries or unbounded MemTable memory waste.
* **Deep**: Why can WAL records physically remain on disk after a durable SSTable flush?
  - **Answer**: WAL segments are rotated in discrete physical chunks (e.g. 64 MB per segment) rather than per-record or per-flush. A single WAL segment may contain records spanning multiple MemTable generations, or an asynchronous MemTable flush may complete and persist to disk while subsequent writes are already appending to the same active segment. Furthermore, obsolete WAL segments can only be unlinked once the MANIFEST transaction acknowledging the flush has durably fsynced. A crash between the MANIFEST fsync and WAL unlinking leaves physically valid but logically obsolete records in the log.
* **Deep**: Why must old WAL records preceding the manifest checkpoint still be integrity-validated rather than skipped immediately?
  - **Answer**: Skipping unverified disk blocks because their claimed sequence or header says `seq <= checkpoint` would hide catastrophic silent corruption (e.g. bit-rot, controller write errors, or malicious tampering) in historical segments. In Lattice, every WAL record must successfully pass length bounding, framing verification, and CRC32-IEEE calculation (`P07-S02-M01-INV-02`) before its sequence number is evaluated. If a pre-checkpoint record suffers a checksum mismatch or corrupted record type, recovery fails closed immediately with `ErrChecksumMismatch`. The storage engine refuses to boot on a physically corrupt log, preserving data safety.
* **Deep**: Why must replay preserve physical sequence order and why must the recovered MemTable be privately built before publication?
  - **Answer**: Replay must process records in strict physical WAL order because operations on the same user key represent chronological state transitions ($K \to V_1 \to V_2 \to \text{TOMBSTONE}$). Inserting records out of order would violate causal ordering and could resurrect deleted keys or overwrite newer values. Building the recovered MemTable in private memory (`P07-S02-M01-INV-09`) guarantees state isolation: if WAL replay encounters corruption at record 300 out of 500, the partially populated MemTable is immediately discarded and live Engine state remains pristine, preventing partial or inconsistent data from ever being queried.
* **Deep**: How does WAL recovery handle atomic batch records (`BATCH_START` / `BATCH_COMMIT`) across crash boundaries?
  - **Answer**: Atomic multi-operation batches in Lattice begin with a `BATCH_START` marker and conclude with a `BATCH_COMMIT` marker. During recovery, the engine replay sink buffers batch operations in an isolated staging list. If all batch records are valid and `BATCH_COMMIT` is encountered, all staged records newer than the checkpoint are atomically committed into the recovery MemTable. If the WAL ends abruptly (EOF) while a batch is still open (`isBatchOpen == true`), the incomplete batch represents an uncommitted in-flight transaction that was interrupted by the crash; the recovery sink drops the staged uncommitted writes without inserting them into the MemTable, maintaining strict all-or-nothing transaction atomicity.
* **Deep**: Why do crash-window temporary files exist in an LSM-tree database engine?
  - **Answer**: LSM-trees enforce atomic SSTable publication. During MemTable flushes or compaction runs, writes stream into a temporary staging file (`.tmp_<filenum>.sst_<random>`) in the parent directory. Only after all data blocks, meta-index, index, filter, and footer are written, `fsync`'d, and closed does the engine execute an atomic hard-link (`os.Link`) or pointer swap to publish the final `.sst` file. If the OS powers off or the process is terminated (`SIGKILL`) mid-write or before `Link` completes, the staging file remains stranded on disk as an unreferenced orphan.
* **Deep**: Why is temporary staging file GC strictly decoupled from SSTable garbage collection?
  - **Answer**: SSTable garbage collection requires analyzing live reference counts across active `Version` levels ($L_0..L_6$), in-flight snapshot iterators, and compaction inputs to identify obsolete SSTables whose keys have been superseded or compacted away. In contrast, temporary-file GC deals strictly with unfinalized, uncommitted staging artifacts that were never part of any `Version`. Conflating the two would risk accidentally deleting unreferenced persistent `.sst` files (e.g. newly created SSTables awaiting MANIFEST commit), violating the core durability invariant.
* **Deep**: Why is exact allowlist grammar matching (`.tmp_<name>.sst_<random>`) vastly safer than generic substring matching (`.*tmp.*`)?
  - **Answer**: Broad regexes like `.*tmp.*` or prefix checks like `tmp_` are catastrophic in production databases. An administrator might store an emergency backup `important.tmp.backup`, or a subsystem might use a dedicated metadata staging file with its own recovery contract (`CURRENT.tmp`). Matching generic `.tmp` strings would blindly delete critical persistent files. Lattice strictly constrains candidate selection via `IsOrphanStagingFile` to `.tmp_<name>.sst_<random>`, ensuring only genuine SSTable staging residue is eligible for deletion while all near-misses and unknown files are preserved.
* **Deep**: Why must symlink candidates never be followed or unlinked during orphan file garbage collection?
  - **Answer**: In multi-tenant environments or shared hosting directories, an attacker could stage an arbitrary symlink masquerading as a temporary file (e.g. `.tmp_000042.sst_attack -> /etc/passwd` or `/data/victim.db`). If the cleanup collector traversed or called `os.Stat` on the symlink, it could delete or mutate external files outside the database directory. Lattice enforces descriptor-level `os.Lstat` inspection before deletion: if `info.Mode()&os.ModeSymlink != 0`, the candidate is immediately rejected as a security violation, the link is never traversed, and the external victim remains completely untouched.
* **Deep**: Why does a startup-only cleanup window simplify concurrency correctness and eliminate writer races?
  - **Answer**: During active database operation, background flush workers and compaction threads continuously create legitimate staging files. Running temporary file GC concurrently without distributed coordination risks deleting active, in-progress flush staging files, causing unexpected write stalls or crash loops. Restricting orphan GC to the startup recovery window (after MANIFEST and WAL replay, but strictly before normal client writes or background flushes commence) guarantees that zero legitimate temporary files are in active use, making all matching candidates unambiguously safe to purge without complex locking protocols.
* **Deep**: Why is a conservative false negative (leaving one orphan `.tmp` file) vastly preferable to a false positive (deleting a valid file)?
  - **Answer**: A false negative costs a few megabytes of recoverable disk space that can be flagged or manually purged later. A false positive deletes live customer data, corrupts an active `VersionSet`, or wipes out a critical metadata file, resulting in unrecoverable catastrophic data loss. In systems engineering, storage reclamation must always fail conservative: when in doubt, preserve the file and log a diagnostic rather than deleting it.
...
* **Mutex Contention & Cache Sharding**: A single global LRU cache protected by a single mutex creates extreme lock contention across multi-core CPUs. Sharding the cache into 16 independent `LRUShard` instances eliminates contention, allowing concurrent queries on different shards to execute with zero lock interference.
* **Murmur3_128 Deterministic Routing**: Block keys `(FileNum, Offset)` are hashed via `Murmur3_128` without heap allocations over a 16-byte fixed-width big-endian buffer. The 4 high bits of the upper 64-bit word (`int((h1 >> 28) & 0x0F)`) route the key to exactly one of the 16 shards (`0 <= idx < 16`).
* **False Sharing & 128-Byte Stride Padding**: Placing two cache shard structs on adjacent memory locations causes CPU cache-line bouncing if both shards share a 64-byte hardware cache line. We wrap `LRUShard` (80 bytes) in a `paddedShard` struct with 48 bytes of trailing padding, enforcing a 128-byte stride ($2 \times 64\text{B}$). This guarantees adjacent shard mutexes never share a cache line.
* **Model A Global Capacity Semantics**: Total capacity $C$ is distributed evenly: `base = C / 16`. The remainder `rem = C % 16` is allocated to the first `rem` shards (shards 0 to `rem - 1`). The total effective capacity matches the configured budget exactly (zero memory amplification).
* **Shard-Local Recency Order**: LRU eviction order is local to each shard. There is no global recency order across all 16 shards, avoiding cross-shard synchronization.
...
* **Intermediate**: How does sharding an LRU cache reduce lock contention?
  * *Answer*: With a single mutex, all concurrent reads and writes serialize. In a 16-way sharded cache, keys are deterministically routed via `Murmur3_128` across 16 independent shards. Goroutines accessing blocks in different shards acquire completely independent mutexes without blocking each other. In our benchmarks with 64 concurrent workers, 16-way sharding reduced median operation latency from ~785 ns/op down to ~498 ns/op (a 36.5% reduction) and scaled throughput linearly.
* **Deep**: What is false sharing in multi-threaded systems, and how do you prevent it in Go?
  * *Answer*: False sharing occurs when two independent variables (like mutexes or counters of adjacent shards) reside on the same hardware cache line (typically 64 bytes). When CPU core A modifies shard 0's mutex, CPU cache coherence protocols (MESI/MOESI) invalidate the entire 64-byte line on core B running shard 1, forcing costly L3/RAM reloads even though the cores are accessing different data structures. In Lattice, we wrap each `LRUShard` (80 bytes) in a `paddedShard` struct containing `_ [48]byte` of trailing padding, ensuring each shard occupies exactly 128 bytes ($2 \times 64\text{B}$). Pointer arithmetic in our test suite verifies that `&shards[1] - &shards[0] == 128`, proving adjacent shard mutexes can never share a cache line.
...
* **"Did You Actually Build This?"**: Does your 16-way sharded cache maintain a global LRU recency order?
  * *Answer*: No. Recency and eviction are local to each shard. A global LRU order would require either a global lock or cross-shard synchronization on every read hit, which completely defeats the purpose of sharding. In production storage engines (LevelDB, RocksDB, Pebble), shard-local LRU is standard practice because uniform hashing across shards ensures that per-shard eviction closely approximates global LRU.
* **"Did You Actually Build This?"**: How does your sharded cache distribute capacity when the total capacity is not evenly divisible by 16?
  * *Answer*: We use Model A deterministic integer distribution: `base = capacity / 16` and `rem = capacity % 16`. The first `rem` shards (shards `0` through `rem - 1`) receive `base + 1` capacity, while the remaining shards receive `base`. This guarantees that the sum of all 16 shard capacities is strictly equal to the requested total capacity, eliminating accidental 16x memory amplification. If `capacity == 0`, all 16 shards are initialized with capacity 0 (non-retaining mode where `Put` immediately drops the block and `Get` always returns a miss).
...
### Concepts I Must Personally Understand
* **P10-S01-M01 Thin Engine**: `Engine` owns active/imm bookkeeping, `nextSeqNum`/`nextFileNum`, lifecycle state, and backpressure accounting; it coordinates WAL, `VersionSet`, transient `TableReader`s, and one shared `ShardedCache` without duplicating them. `Put`/`Delete` allocate one sequence, `AppendSync` WAL before MemTable (success only after both); `Get` checks active → imm newest-first via `Iterator.Seek` OpType (tombstone stops search) → pinned `Version` L0 newest-`FileNum`-first then L1.. range-pruned via transient readers + shared cache. `Open()` mkdirs, `RecoverWAL`s (quiescent first), then opens `RotatingWriter`; `Close()` freezes + closes WAL + backpressure only.
* **P10-S01-M02 Async Flusher**: 64MB `ByteSize()` threshold (`memtable.MaxMemTableSize`) triggers atomic `Freeze` + append-to-`immMems` + fresh active under `Engine.mu` (no I/O); single `flushLoop` (coalesced `flushCh`, `stopCh`, `WG`) flushes oldest-first without Engine lock via `TableWriter` order-preserving stream (original seqs/tombstones), `Finish`, `LogAndApply(AddFile L0, NextFileNum, LastSeqNum)`; imm retires only after publication (no gap); failures retain imm + `FlushError`; `Close()` stops worker, waits in-flight, then closes WAL/manifest (no M04 drain).
* **P10-S01-M03 L0 Pacing/Stall**: Pressure is `Current().NumFiles(0)` per gate (no duplicate counter); ≤8 free, 9/10/11/12 → 1/5/15/30ms one-shot pacing, >12 stall in 20ms polls until ≤12. Gate sits after validation, before `Acquire`/seq/WAL/mem (no reserved seqs, no internal copies while waiting, no locks held); `Get` never gated; relief comes from compaction reducing L0; waits honor ctx/`stopCh`/close with `ErrWriterClosed`.
* **P10-S01-M04 Clean Shutdown**: `Close()` is single-winner RUNNING→CLOSING→CLOSED with a remembered terminal error: reject new writes, close `stopCh` (M03 gates exit), join the flusher after its in-flight flush, synchronously drain final-rotated active plus every queued imm oldest-first via `flushOne` (skipped when the worker never started), then freeze + close WAL/manifest/backpressure first-error-wins. `Get` keeps serving; failures retain state with WAL recovery intact; external compaction is serialized by existing locks (callers quiesce publishers before Close).
