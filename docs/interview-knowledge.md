...
* **"Did You Actually Build This?"**: How do you measure the empirical false positive rate of your Bloom filter implementation to verify it matches theoretical expectations?
  > **Model Answer**: "I constructed a test filter with $n = 10,000$ keys, populated it with 10,000 distinct binary keys, and first proved zero false negatives (all 10,000 returned true). Next, I queried 10,000 distinct non-existent keys generated from a disjoint key space and counted the hits. The observed false positive rate was ~0.70%, cleanly matching the theoretical prediction of 0.82%. In Sub-Phase 05.2, this is scaled to a formal benchmark across 1,000,000 absent keys asserting $p \le 0.01$."
