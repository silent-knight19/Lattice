...
* **Limitation & Architectural Boundaries**:
  ...
  1. *Production TLS 1.3 & Mandatory Client mTLS Transport Security (SEC-P11-001)*: The TCP server supports production TLS 1.3 transport encryption. For external/non-loopback listeners, TLS strictly mandates mutual TLS (mTLS) with trusted client CA verification and mandatory client certificate enforcement (`ServerConfig.TLSCertFile`, `TLSKeyFile`, `ClientCAFile`, `RequireClientCert`). Ordinary one-way TLS without client certificates is permitted exclusively on loopback interfaces (`127.0.0.1`, `localhost`, `::1`) for local development and test automation. Legacy protocols (TLS 1.0–1.2) are strictly rejected fail-closed. Unencrypted plaintext transport remains strictly restricted to loopback addresses by default; running plaintext on non-loopback interfaces requires explicit opt-in via `ServerConfig.InsecureTransport = true`. Automated online background certificate rotation is intentionally out of scope (certificate updates take effect upon process restart or transport reinitialization).
  ...
  5. *Supported Engine Operations*:
     - `OP_PUT`, `OP_GET`, `OP_DELETE`, `OP_BATCH`, `OP_EXISTS`, and `OP_STATS` dispatch directly to `Engine.Put`, `Engine.Get`, `Engine.Delete`, `Engine.Batch`, `Engine.Exists`, and `Engine.Stats` in standalone mode, or through consensus `ProposalRouter` / `ReadRouter` in cluster mode.
     - `OP_STATS` returns an authoritative, bounded point-in-time diagnostic JSON snapshot aggregating engine state, memtable/backpressure memory metrics, sstable/wal storage metrics, block cache hit ratios, active connections, and Raft consensus metadata (when clustered). STATS never mutates storage state or performs expensive disk scans.
  ...
  7. *Slowloris & Resource Defense*:
     ...
     - `MaxConnections` (default 4096) caps simultaneous accepted connections to mitigate OS file descriptor and goroutine exhaustion.
...
* **Limitation & Architectural Boundaries**:
  ...
  2. *Strict Protocol Fidelity (No Client-Side Operation Fabrication)*: Commands `PUT`, `GET`, `DELETE`, `BATCH`, `EXISTS`, and `STATS` execute against active storage engine operations and authoritative Raft routers. For `STATS`, the client transmits the real wire opcode (`OpStats`) to the server and transparently renders the server's authoritative diagnostic JSON snapshot. The client does NOT fabricate boolean values via client-side `GET` emulation or synthesize fake statistics.
...
* **Limitation & Architectural Boundaries**:
  ...
  1. *Injectable Dialing & Production mTLS Transport*: `PeerConnectionManager` manages socket lifecycle, bounded exponential reconnect, TCP keep-alive, write serialization, framed message consumption, and production mutual TLS 1.3 authentication (`PeerTLSCertFile`, `PeerTLSKeyFile`, `PeerCAFile`). The manager validates peer certificate chains against a trusted CA bundle, enforces strict TLS 1.3, verifies peer role separation (`OU = Lattice Raft Peer`), and strictly matches authenticated certificate `NodeID` against the canonical cluster topology. Local development and offline unit tests operate over loopback TCP with unencrypted transport explicitly scoped to loopback or requiring `--insecure-transport`. Automated online background certificate rotation is intentionally out of scope (certificate replacement takes effect upon process restart or manager reinitialization).
...
* **Limitation & Architectural Boundaries**:
  ...
  1. *Production Peer mTLS & Manager-Owned TLS Architecture*: Production peer transport enforces TLS 1.3 mutual TLS with canonical NodeID binding for all non-loopback peer endpoints. Merely supplying a TLS configuration is insufficient: arbitrary or one-way TLS configurations (e.g. `ClientAuth: NoClientCert`, `RequestClientCert`, `VerifyClientCertIfGiven`, `RequireAnyClientCert`, or nil `ClientCAs`) fail closed with `errors.ErrInsecureTransport`. For non-loopback Raft peers, Lattice strictly owns the TLS authentication boundary. Custom transport injection (`RawDialFunc`, `DialFunc`) cannot replace or bypass Lattice's TLS 1.3, mTLS, peer trust-anchor validation, certificate-role validation, or NodeID binding. Custom dialers act strictly as low-level raw network transport seams; returning a pre-secured or unauthenticated `*tls.Conn` on non-loopback peers is rejected fail-closed. Lattice wraps the raw transport with its own manager-owned TLS 1.3 configuration, executes the TLS handshake against the trusted peer CA pool, validates cryptographic trust chains (`len(VerifiedChains) > 0`), enforces peer role (`OU = Lattice Raft Peer`) and expected remote NodeID binding, and asserts security admission criteria before the connection can ever reach `PeerStateConnected`. Programmatic `TLSConfig` and `ListenerTLSConfig` instances are wrapped with mandatory verifiers that execute even if the caller provides a custom callback. Inbound listeners intercept and wrap `GetConfigForClient` callbacks to ensure dynamic configurations cannot bypass or weaken security (mandating TLS 1.3+, `RequireAndVerifyClientCert`, no `InsecureSkipVerify`, strict `ClientCAs` equality against trusted peer roots, and chained Lattice identity verification). The client-facing `--insecure-transport` flag applies strictly to the client data plane and DOES NOT permit unauthenticated or plaintext peer consensus communication. Peer `InsecureTransport` is loopback-only. Any non-loopback peer connection without mutual TLS fails closed with `errors.ErrInsecureTransport` regardless of the `InsecureTransport` setting. Unencrypted peer transport is permitted strictly for loopback test and development topologies. Automated online certificate rotation is intentionally out of scope (requires node restart).
  ...
  8. *Client-Facing `GetConfigForClient` mTLS Boundary & Post-Handshake Verification*: Client-facing TCP server transport (`:9099`) eliminates dynamic `GetConfigForClient` trust-relaxation bypasses via `WrapClientServerTLSConfig`. Handshakes mandate TLS 1.3+, `RequireAndVerifyClientCert`, no `InsecureSkipVerify`, strict `ClientCAs` equality against trusted client roots, and chained Lattice PKI separation (rejecting Raft peer certificates on client transport). Dynamic configurations cannot strip or bypass caller-installed base `VerifyConnection` checks. Across all server lifecycle paths (`NewServer`, `Listen`, `Serve`), the security wrapper is consistently applied. In `handleConn`, post-handshake defense-in-depth independently validates connection state, TLS version, peer certificates, and cryptographically verifies the client leaf certificate against `ServerConfig.TLSConfig.ClientCAs`, ensuring that unauthenticated or rogue connections are dropped fail-closed with 0 application requests processed.
* **Why It Exists**:
...
* **Limitation & Architectural Boundaries**:
  ...
  8. *Production mTLS*: Peer transport supports TLS 1.3 mutual TLS with NodeID verification and PKI separation; loopback plaintext remains available for local testing. Automated online certificate rotation is out of scope.
* **Why It Exists**:
...
* **Limitation & Architectural Boundaries**:
  ...
  6. *Peer Transport Security*:
     - Peer TCP communication supports full mutual TLS 1.3 with peer identity verification bound to cluster topology, strict PKI separation from client certificates (`OU = Lattice Raft Peer`), and fail-closed handshake rejection. Plaintext peer communication is restricted to loopback interfaces unless `--insecure-transport` is explicitly configured. Automated online certificate rotation is intentionally out of scope.
* **Why It Exists**:
