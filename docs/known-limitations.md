...
* **Limitation & Architectural Boundaries**:
  ...
  1. *Production TLS 1.3 & mTLS Transport Security (SEC-P11-001)*: The TCP server supports production TLS 1.3 transport encryption with server certificate authentication and optional client mutual TLS (mTLS) verification (`ServerConfig.TLSCertFile`, `TLSKeyFile`, `ClientCAFile`, `RequireClientCert`). Legacy protocols (TLS 1.0–1.2) are strictly rejected fail-closed. Binding to non-loopback wildcard (`0.0.0.0`, `:port`) or external interfaces is permitted when TLS is configured. Unencrypted plaintext transport remains strictly restricted to loopback addresses (`127.0.0.1`, `localhost`, `::1`) by default for local development and testing; running plaintext on non-loopback interfaces requires explicit `ServerConfig.InsecureTransport = true`. Automated online background certificate rotation is intentionally out of scope (certificate updates take effect upon process restart or transport reinitialization).
...
* **Limitation & Architectural Boundaries**:
  ...
  1. *Production Peer mTLS & Plaintext Public Network Restriction*: Production peer transport enforces TLS 1.3 mutual TLS with canonical NodeID binding. Unencrypted cleartext TCP dialing of remote non-loopback addresses without setting `PeerConnectionConfig.InsecureTransport = true` fails closed with `errors.ErrInsecureTransport`. Automated online certificate rotation is intentionally out of scope (requires node restart).
