...
* **Limitation & Architectural Boundaries**:
  ...
  1. *Production Peer mTLS & Independent Security Boundary*: Production peer transport enforces TLS 1.3 mutual TLS with canonical NodeID binding for all non-loopback peer endpoints. Merely supplying a TLS configuration is insufficient: arbitrary or one-way TLS configurations (e.g. `ClientAuth: NoClientCert`, `RequestClientCert`, `VerifyClientCertIfGiven`, `RequireAnyClientCert`, or nil `ClientCAs`) fail closed with `errors.ErrInsecureTransport`. Programmatic peer configurations (`TLSConfig`, `ListenerTLSConfig`, `DialFunc`) are subject to the same mandatory-mTLS validation. Programmatic peer TLS configurations are not trusted merely because they satisfy generic TLS/mTLS requirements. Lattice always applies its mandatory peer certificate role and NodeID/topology identity validation. A caller-supplied `VerifyConnection` callback cannot disable or replace Lattice validation; caller verification and Lattice mandatory verification are both executed, with Lattice checks taking strict precedence. Custom `DialFunc` hooks cannot bypass the peer security contract on non-loopback networks: non-loopback connections mandate valid TLS 1.3 mTLS and certificates are verified post-dial. Outbound peer certificates are strictly bound to expected target NodeIDs, and inbound peer certificates are strictly bound to cluster topology, self-connection rejection, and first-frame consensus message identity. The client-facing `--insecure-transport` flag applies strictly to the client data plane and DOES NOT permit unauthenticated or plaintext peer consensus communication. Peer `InsecureTransport` is loopback-only. Any non-loopback peer connection without mutual TLS fails closed with `errors.ErrInsecureTransport` regardless of the `InsecureTransport` setting. Unencrypted peer transport is permitted strictly for loopback test and development topologies. Automated online certificate rotation is intentionally out of scope (requires node restart).
