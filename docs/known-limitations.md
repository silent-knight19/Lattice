...
* **Limitation & Architectural Boundaries**:
  ...
  1. *Production Peer mTLS & Independent Security Boundary*: Production peer transport enforces TLS 1.3 mutual TLS with canonical NodeID binding for all non-loopback peer endpoints. The client-facing `--insecure-transport` flag applies strictly to the client data plane and DOES NOT permit unauthenticated or plaintext peer consensus communication. Any non-loopback peer connection without mutual TLS fails closed with `errors.ErrInsecureTransport` regardless of the `InsecureTransport` setting. Unencrypted peer transport is permitted strictly for loopback test and development topologies. Automated online certificate rotation is intentionally out of scope (requires node restart).
