...
1. **Client Transport (`:9099`)**:
   ...
   - **`GetConfigForClient` Security Invariants on Client Transport**: To eliminate the TLS configuration escape hatch on client-facing listeners, `WrapClientServerTLSConfig` strictly intercepts and wraps caller-provided `GetConfigForClient` callbacks. Any dynamic `*tls.Config` returned during the TLS handshake is subjected to mandatory security invariants before admission: it cannot downgrade TLS below 1.3 (`MinVersion >= VersionTLS13`, preserving explicit ceiling), cannot enable `InsecureSkipVerify`, cannot weaken client authentication (`ClientAuth == RequireAndVerifyClientCert`), cannot substitute or omit the trusted client `ClientCAs` pool (verified via `dynCfg.ClientCAs.Equal(trustedClientCAs)` and pinned to the trusted pool), and automatically inherits Lattice's client identity and PKI separation verification. Any caller-provided base `VerifyConnection` is preserved and chained; dynamic configurations cannot strip or bypass verification callbacks. Rogue CA substitutions, omitted verifiers, weakened authentication modes, or TLS downgrades trigger immediate handshake failure.
   - **Server Lifecycle & Post-Handshake Defense-in-Depth**: Both `NewServer -> Listen()` and `NewServer -> Serve(listener)` enforce identical security wrapping. In `handleConn`, post-handshake defense-in-depth independently validates `HandshakeComplete`, TLS 1.3+, peer certificates present and verified, PKI separation, and cryptographic verification of the client leaf certificate against `ServerConfig.TLSConfig.ClientCAs`, guaranteeing that unauthenticated or rogue clients fail closed with 0 application requests processed.
...
2. **Raft Consensus Transport (`:9098`)**:
   - **Mandatory Peer mTLS**: All peer-to-peer Raft cluster communication on non-loopback networks mandates mutual TLS 1.3 with peer certificate verification (`--peer-tls-cert`, `--peer-tls-key`, `--peer-ca`). Merely supplying a TLS configuration is insufficient: arbitrary or one-way TLS configurations (e.g., `ClientAuth: NoClientCert`, `RequestClientCert`, `VerifyClientCertIfGiven`, `RequireAnyClientCert`, or nil `ClientCAs`) are strictly rejected fail-closed.
   - **Independent Security Boundary**: The client-facing `--insecure-transport` flag applies strictly to the client data plane; it DOES NOT disable or downgrade Raft peer transport authentication. Non-loopback peer communication without valid mutual TLS 1.3 fails closed at configuration validation, programmatic manager initialization, and listener startup. Peer `InsecureTransport` is loopback-only.
   - **Programmatic Configuration & Manager-Owned TLS Architecture**: For non-loopback Raft peers, Lattice strictly owns the TLS authentication boundary. Custom transport injection (`RawDialFunc`, `DialFunc`) cannot replace or bypass Lattice's TLS 1.3, mTLS, peer trust-anchor validation, certificate-role validation, or NodeID binding. A custom dialer acts strictly as a low-level raw network transport seam (e.g. for connection tracking or partition simulation); returning a pre-secured or unauthenticated `*tls.Conn` on non-loopback peers is rejected fail-closed. Lattice wraps the raw transport with its own manager-owned TLS 1.3 configuration, executes the TLS handshake against the trusted peer CA pool, validates cryptographic trust chains (`len(VerifiedChains) > 0`), enforces peer role (`OU = Lattice Raft Peer`) and expected remote NodeID binding, and asserts security admission criteria before the connection can ever reach `PeerStateConnected`. Programmatic `TLSConfig` and `ListenerTLSConfig` instances are wrapped with mandatory verifiers that execute even if the caller provides a custom callback.
   - **`GetConfigForClient` Security Invariants**: To eliminate the TLS configuration escape hatch on inbound peer listeners, `WrapPeerServerTLSConfig` strictly intercepts and wraps caller-provided `GetConfigForClient` callbacks. Any dynamic `*tls.Config` returned during the TLS handshake is subjected to mandatory security invariants before admission: it cannot downgrade TLS below 1.3 (`MinVersion >= VersionTLS13`), cannot enable `InsecureSkipVerify`, cannot weaken client authentication (`ClientAuth == RequireAndVerifyClientCert`), cannot substitute or omit the trusted peer `ClientCAs` pool (verified via `dynCfg.ClientCAs.Equal(trustedClientCAs)` and pinned to the trusted pool), and automatically inherits Lattice's inbound peer identity verification (`VerifyConnection`). Rogue CA substitutions, omitted verifiers, or weakened auth modes in dynamic configurations trigger immediate handshake failure and connection termination, preventing the connection from ever reaching `PeerStateConnected`.
   - **Node Identity Pinning & PKI Separation**: Each cluster node presents an X.509 certificate with `OU = Lattice Raft Peer` whose asserted NodeID (SPIFFE URI `spiffe://lattice/node/<id>`, SAN DNS `node-<id>`, or CN `node-<id>`) is strictly validated against the canonical cluster topology. Untrusted, expired, mismatched, or ordinary client certificates are dropped immediately before processing any consensus frames. Loopback-only peer testing may utilize unencrypted transport exclusively in local/test topologies. Automated online certificate rotation is intentionally out of scope (requires node restart).
