...
2. **Raft Consensus Transport (`:9098`)**:
   ...
   - **Programmatic Configuration & Custom Dialer Validation**: Programmatic peer TLS configurations (`TLSConfig`, `ListenerTLSConfig`, `DialFunc`) are subject to the identical mandatory-mTLS validation rule (`ValidatePeerTLSConfig`). Supplying a custom `DialFunc` does not bypass peer transport invariants.
