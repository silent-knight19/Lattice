...
* **P11-S01-M02: TCP Listener & Goroutine Connection Pool**
  ...
  * *Changes*: `Server`, `NewServer`, `Server.Listen(addr string) error`, `Server.Serve(l net.Listener) error`, `Server.Shutdown(ctx context.Context) error`, `Server.Close() error`, Slowloris defense (`HeaderTimeout`, `PayloadTimeout`, `IdleTimeout`, `WriteTimeout`), `MaxConnections` guard, request dispatch to `Engine.Put`, `Engine.Get`, `Engine.Delete`, `Engine.Batch` (or consensus `ProposalRouter` in cluster mode), deterministic unsupported error responses for `EXISTS`, `STATS`, error sanitization, and connection tracking.
