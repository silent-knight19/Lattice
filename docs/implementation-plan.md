...
| **SEC-003 / F-003** | `internal/version` (`ActiveVersions`) | **MEDIUM** | Unsafe `ActiveVersions()` lifetime race returning unpinned version pointers subject to post-unlock finalization | **REMEDIATED** | Ownership contract test (`TestVersionSet_ActiveVersions_OwnershipContract`), deterministic race test (`TestVersionSet_ActiveVersions_DeterministicLifetimeRace`), finalization protection test, concurrent stress test, randomized lifecycle test, benchmarks (~12-26 ns/op), full race suite clean. Phase 07 remains BLOCKED. |
| **SEC-004 / F-004** | `internal/version` (`ValidateFileMetadata`, `AddFile`, `DecodeVersionEdit`) | **MEDIUM** | VersionEdit `AddFile` and manifest decoder accept structurally/semantically invalid `FileMetadata` (FileNum=0, FileSize=0, backwards sequence/key ranges, malformed/truncated InternalKeys) | **REMEDIATED** | Semantic validation suite (`TestSEC004_ValidMetadata`, `TestSEC004_InvalidScalars`, `TestSEC004_InvalidKeys`, `TestSEC004_KeyRangeOrdering`), atomicity test (`TestSEC004_Atomicity`), defensive copying test (`TestSEC004_DefensiveCopying`), decoder corruption test (`TestSEC004_DecoderRejection`), adversarial matrix (`TestSEC004_AdversarialTable`), admission parity test (`TestSEC004_AdmissionParity`), fuzz testing (`FuzzDecodeVersionEdit`, 1.42M execs, 0 crashes, all invariants asserted on admitted entries), full race suite clean. Phase 07 remains BLOCKED. |
| **SEC-005 / F-005** | `internal/version` (`Version.Ref`, `Version.TryRef`) | **LOW** | Version reference counter integer overflow at `math.MaxInt32` wrapping to `math.MinInt32` | **REMEDIATED** | Boundary tests (`TestSEC005_TryRef_AtMaxInt32`, `TestSEC005_Ref_AtMaxInt32`, `TestSEC005_OneBelowMaximum`, `TestSEC005_MaxRelease`, `TestSEC005_BoundaryRoundTrip`), dead/exhausted distinction test (`TestSEC005_DeadVersionDistinction`), underflow regression, 50-worker concurrent collision test (`TestSEC005_ConcurrentCollision`), concurrent stress test (`TestSEC005_ConcurrentStress`), arithmetic property test, benchmarks (~8 ns/op), full race suite clean. Phase 07 remains BLOCKED. |
| **SEC-006 / F-006** | `internal/sstable` (`TableWriter`, `NewTableWriter`, `Finish`, `cleanupStaging`) | **MEDIUM** | TableWriter parent-directory symlink / TOCTOU race: pathname-based directory operations subject to symlink injection, parent directory substitution, intermediate path component redirect, and cleanup deletion in swapped directories | **REMEDIATED** | Parent symlink rejection (`TestSEC006_ExistingParentSymlink_Rejected`), parent replacement detection (`TestSEC006_ParentDirectoryReplaced_Detected`), symlink swap detection (`TestSEC006_ParentDirectorySymlinkSwap_Detected`), intermediate symlink redirect (`TestSEC006_IntermediateSymlinkRedirect_Detected`), intermediate symlink at construction (`TestSEC006_IntermediateSymlink_InitialRejection`), destination symlink (`TestSEC006_DestinationSymlink_Rejected`), staging symlink (`TestSEC006_StagingSymlink_Rejected`), cleanup isolation (`TestSEC006_FailureCleanup_DirectoryIsolation`), pinned directory sync (`TestSEC006_PinnedDirectorySync`), concurrent replacement race 15 iterations (`TestSEC006_ConcurrentReplacementRace`), 9-scenario object identity matrix (`TestSEC006_ObjectIdentityMatrix`), full race suite clean. Phase 07 remains BLOCKED. |
| **SEC-007 / F-007** | `internal/sstable` (`TableReader`, `NewTableReader`, `NewTableReaderWithFile`) | **LOW** | TableReader path / symlink TOCTOU: pathname-based SSTable opening subject to symlink following, intermediate directory redirection, and in-flight inode substitution | **REMEDIATED** | Pre-open Lstat inspection, intermediate component validation (`validatePathNoSymlinks`), descriptor Stat validation (`fstat`), post-open Lstat re-inspection, inode pinning invariance (`os.SameFile(fstat, lstatBefore)` and `os.SameFile(fstat, lstatAfter)`), descriptor-centric `ReadAt` execution, leak-free deferred cleanup, 16 test cases (`TestSEC007_*`), 1.47M+ fuzz iterations (`FuzzNewTableReader_Paths`), full race suite clean. Phase 00-06 audit blockers CLEARED. Phase 07 NOT STARTED. |
...
* **P08-S03-M02: Atomic Manifest Commit & Obsolete File Deletion**
  * *Objective*: Commit `VersionEdit` deleting compaction input files and adding output files, atomically publish the new immutable `Version`, and safely unlink obsolete SSTables once no longer pinned by any active `Version`.
  * *Changes*: `internal/version/version_set.go` (`LogAndApply`, `applyMu`, `collectObsoleteFilesLocked`, `deletePhysicalFiles`, `CleanObsoleteFiles`), `internal/version/version.go` (`Version.finalize` hook for obsolete file reclamation), `internal/version/version_apply.go` (`applyEditToVersion`), `internal/errors/errors.go` (`ErrNilVersionEdit`), `internal/version/log_and_apply_test.go`, `internal/version/log_and_apply_differential_test.go`, `internal/version/log_and_apply_fuzz_test.go`, `internal/version/log_and_apply_bench_test.go`.
  * *Invariants*: Strict hardware durability boundary (`fdatasync`/`f.Sync` barrier verified before in-memory publication); atomic state transition (`vs.mu.Lock()` assigns `Current`, advances scalar watermarks, and enqueues deleted file numbers); fail-closed manifest failure semantics (manifest write/sync error poisons writer, discards uninstalled Version, and preserves existing Current intact); stale-base and race prevention (`vs.applyMu` serializes commits while readers holding `vs.mu.RLock()` read `Current()` without disk I/O blocking); safe obsolete SSTable unlinking (files unlinked only when absent from all active versions in the circular doubly-linked chain and refCount transitions to 0); non-rollback cleanup failures (unlinking failure logs diagnostic error and does not invalidate published Version); 100% recovery parity (`ReplayManifest` from scratch produces identical levels, FileMetadata, and scalar watermarks).
  * *Tests*: Full acceptance matrix A-V (empty edit, add file, delete file, compaction replacement, multiple adds/deletes, manifest append failure, manifest sync failure, invalid edit rejection, scalar regression rejection, duplicate file number rejection, cross-level file collision rejection, missing physical output rejection, old-version reader pin preservation, cleanup after final unref, cleanup failure resilience, concurrent `LogAndApply` multi-worker stress, crash/replay equivalence, manifest replay rejection parity, idempotence/retry behavior, `Current` pinning during publication, `ActiveVersions` co-existence, and file cleanup race under `-race`), 2,000-iteration randomized differential test suite vs independent reference model (0 mismatches), native Go fuzz target (`FuzzLogAndApply`, 13,100+ execs, 0 crashes), and benchmarks measuring end-to-end commit latency, in-memory version derivation overhead (~13.3 µs/op), and manifest write sync overhead (~3.7 ms/op).
  * *Completion*: 100% tests pass under `-race`, 0 data races, `go vet` clean, `golangci-lint` clean (0 issues). Sub-Phase 08.3 M02 verified.
...
* **P09-S01-M02: 16-Way Sharded Cache Partitioning with Cache-Line Padding**
  * *Objective*: Partition block caching across 16 independent `LRUShard` instances using `Murmur3_128` high-bit routing; pad shard structures to eliminate hardware cache-line false sharing under multi-core concurrency.
  * *Changes*: `internal/cache/sharded.go` (`ShardedCache`, `ShardedBlockCache`, `NumShards = 16`, `CacheLineSize = 64`, `PaddedShardSize = 128`, `paddedShard`, `ShardIndex(BlockKey) int`, `NewShardedCache`, `NewShardedBlockCache`, `Get`, `GetBlock`, `Put`, `PutBlock`, `Peek`, `Contains`, `Remove`, `Len`, `Capacity`, `Clear`, `Shard`), `internal/cache/lru.go` (`initLRUShard` helper for in-place array initialization).
  * *Invariants*:
    * Fixed 16-way sharding architecture (`NumShards = 16`).
    * Deterministic non-allocating `Murmur3_128` routing over 16-byte fixed-width binary encoding (`[8]byte FileNum BigEndian || [8]byte Offset BigEndian`), mapped via `int((h1 >> 28) & 0x0F)`.
    * Model A global capacity semantics: Total capacity $C$ split into `base = C/16` per shard, with `rem = C%16` remainder capacity distributed to the first `rem` shards (sum of shard capacities $\equiv C$). Capacity 0 creates 16 zero-capacity non-retaining shards.
    * Cache-line padding & alignment: `paddedShard` embeds `LRUShard` (80 bytes) with 48 bytes trailing padding (`[48]byte`), enforcing a stride of exactly 128 bytes ($2 \times 64\text{B}$). This guarantees adjacent shard mutexes never share a 64-byte hardware cache line.
    * Concurrency & lock granularity: Zero global mutex on hot read/write paths (`Get`, `Put`, `Peek`, `Contains`, `Remove` lock only the target shard). Read hits/misses and writes across distinct shards proceed 100% in parallel. Shard recency is local; no global recency order is maintained.
    * Memory isolation: Strict defensive copying via `bytes.Clone` on entry and exit inherited from `LRUShard`.
  * *Tests*: Full acceptance matrix (constructor parameter checks, deterministic capacity distribution for $C \in \{0, 1, 15, 16, 17, 50, 64, 100, 1024\}$, memory stride verification between adjacent shards $\ge 64\text{B}$ and $= 128\text{B}$, routing determinism and stability, distribution uniformity across 10,000 deterministic keys [min 574, max 673 per shard; mean 625], cross-SSTable key isolation `(1, X) != (2, X)`, complete CRUD semantics, input/output buffer mutation isolation, concurrent multi-goroutine stress with 8, 16, 32, 64 workers under `-race` with 0 races/deadlocks), 15,000-operation differential test suite vs independent slice-based reference model (0 mismatches), native Go fuzz target (`FuzzShardedCache`, 3.49M+ executions, 0 crashes/invariant violations), and microbenchmarks measuring single-threaded operations (`Get_Hit` ~341 ns/op, `Get_Miss` ~12.6 ns/op, `Put_New` ~356 ns/op, `Put_Existing` ~336 ns/op) and multi-core contention scalability (`ShardedCache_16Shards_64Workers` at ~498 ns/op vs `SingleShard_64Workers` at ~785 ns/op, a 36.5% latency reduction and throughput scaling).
  * *Completion*: 100% tests pass under `-race`, 0 data races, `go vet` clean, `golangci-lint` clean (0 issues). Sub-Phase 09.1 M02 verified.

### Sub-Phase 09.2: SSTable Read Path Block Cache Integration
* **P09-S01-M03: SSTable Read Path Block Cache Integration**
  * *Objective*: Wire `ShardedCache` / `BlockCache` into `TableReader.ReadBlock` and `TableReader.Seek` so cached blocks bypass physical disk I/O and checksum calculation while strictly enforcing bounds validation before cache access and fail-closed corruption handling.
  * *Changes*:
    * `internal/sstable/table_reader.go`: Added `BlockCache` interface (`Get(cache.BlockKey) ([]byte, bool)`, `Put(cache.BlockKey, []byte)`), `TableReaderOptions{FileNum uint64, BlockCache BlockCache}`, `fileNum uint64` and `blockCache BlockCache` fields on `TableReader`, `parseTableFilename(string) (uint64, bool)` for automatic `%06d.sst` parsing, `NewTableReaderWithOptions` and `NewTableReaderWithFileAndOptions`, accessors/mutators `FileNum()`, `SetFileNum()`, `BlockCache()`, `SetBlockCache()`.
    * Refactored `validateBlockHandle(handle BlockHandle) error` to strictly validate candidate block bounds against `MaxDataBlockSize`, 64-bit integer limits, and physical file size boundary BEFORE consulting the cache, preventing bounds bypass attacks.
    * Implemented `ReadBlock(handle BlockHandle) ([]byte, error)` and `readBlockLocked(handle BlockHandle) ([]byte, error)`: checks `r.blockCache.Get(key)` where `key = cache.NewBlockKey(r.fileNum, handle.Offset)`. On hit, returns defensive copy; on miss, reads via `readExactAt`, validates minimum 8-byte trailer, verifies CRC32-IEEE checksum, verifies restart count and restart array bounds, populates `r.blockCache.Put(key, blockBuf)`, and returns.
    * Corrupted blocks fail closed with `ChecksumMismatchError` or `DataBlockCorruptedError` and are NEVER inserted into the cache.
    * Backward compatibility: `ReadDataBlock(handle)` delegates to `ReadBlock(handle)`; `Seek(userKey)` utilizes `readBlockLocked(handle)` and delegates to `searchDataBlockVerified` to avoid redundant CRC32 verification on warm blocks.
    * `internal/sstable/export_test.go`: Added `ReadAtFnForTesting()`.
    * `internal/sstable/table_reader_cache_test.go`: 10 comprehensive acceptance tests (cold miss, warm hit with physical disk read avoidance via atomic counter, cache disabled, poison-proof cache hits, corrupted disk blocks never cached, invalid handle bounds rejection before cache lookup, cross-SSTable isolation `(FileNum A, Offset X) != (FileNum B, Offset X)`, caller defensive copy isolation, Seek point lookup caching, TableIterator cache integration, and 64-worker concurrent read stress under `-race`).
    * `internal/sstable/table_reader_cache_differential_test.go`: 5,000 randomized operations comparing Cached Reader vs Direct Reader (0 mismatches, 178.1x reduction in physical disk reads: 2,493 direct vs 14 cached).
    * `internal/sstable/table_reader_cache_fuzz_test.go`: Native Go fuzz target `FuzzTableReader_ReadBlock` (1.85M+ executions in 5.1s, 0 crashes, 0 parity failures).
    * `internal/sstable/table_reader_cache_bench_test.go`: Microbenchmarks demonstrating 6.2x faster `ReadBlock` on warm cache (55.98 ns/op vs 345.7 ns/op), 2.4x faster `Seek` (225.1 ns/op vs 532.4 ns/op), and 7.5x faster throughput under 64-worker concurrency (160.0 ns/op vs 1193 ns/op).
  * *Completion*: 100% tests pass under `-race`, 0 data races, `go vet` clean, `golangci-lint` clean (0 issues). Sub-Phase 09.2 M03 verified.
...
* **P11-S01-M02: TCP Listener & Goroutine Connection Pool**
  ...
  * *Changes*: `Server`, `NewServer`, `Server.Listen(addr string) error`, `Server.Serve(l net.Listener) error`, `Server.Shutdown(ctx context.Context) error`, `Server.Close() error`, Slowloris defense (`HeaderTimeout`, `PayloadTimeout`, `IdleTimeout`, `WriteTimeout`), `MaxConnections` guard, request dispatch to `Engine.Put`, `Engine.Get`, `Engine.Delete`, `Engine.Batch`, `Engine.Exists` (or consensus `ProposalRouter` / `ReadRouter` in cluster mode), deterministic unsupported error responses for `STATS`, error sanitization, and connection tracking.
...
* **P12-S01-M02: Interactive REPL Client (`cmd/lattice-cli`)**
  * *Objective*: Interactive terminal prompt supporting `PUT`, `GET`, `DELETE`, `BATCH`, `EXISTS`, `STATS` commands with optional colored output.
...
* **P17-S01-M01: Leader Quorum Heartbeat Verification** (complete)
  ...
  * *Implementation & Invariants*: Implemented `Node.ReadIndex(ctx context.Context) (ReadIndexResult, error)` and wired into the production multi-node Lattice daemon over `PeerConnectionManager` and framed TCP transport. Enforces strict atomic relationship between leader role, current term, leader epoch, and `commitIndex` at the linearization point. Single-node clusters ($N=1$) satisfy majority immediately (quorum size = 1) without network I/O. Multi-node clusters ($N > 1$) broadcast empty AppendEntries probes with unique 64-bit cryptographic nonces echoed by followers across real TCP sockets. Acknowledgment requires current-term responses matching the round nonce with `resp.Success == true` from a majority ($1 + \text{acks} \ge \text{quorumSize}$). Defends against stale responses (older term, older round, mismatched nonce, `Success=false`), higher terms (forces stepdown), concurrent leadership epoch changes, context cancellation/deadlines, and node closure. Verified end-to-end with 3-node real TCP cluster, partition/heal recovery, 25+ concurrent reads, and zero data races.
* **P17-S01-M02: State Machine Read Barrier Execution** (complete)
  * *Objective*: Wait until local state machine applies up to recorded `commitIndex`, revalidate leadership authority, and serve linearizable `GET` in cluster mode.
  * *Implementation & Invariants*:
    - **Read Barrier Synchronization**: Implemented `Node.WaitForApplied(ctx context.Context, targetIndex LogIndex) error` with dedicated `applyWaiters` notification map protected by `applyWaitMu`. Preserves `applyNotifyCh` exclusively for the internal apply loop to prevent notification stealing and lost wakeups. Monotonically tracks `maxNotifiedApplied` to prevent races where apply advances between checking condition and waiter registration. Immediately unblocks when `lastApplied >= targetIndex`. Unblocks all waiters fail-closed with terminal error on apply loop failure or node closure. Pure context errors (`context.Canceled`, `context.DeadlineExceeded`) are preserved without misclassification.
    - **Leadership Post-Wait Fencing**: Implemented `Node.ValidateLeadership(expectedTerm Term, expectedEpoch uint64) error`. Protects the critical window between `ReadIndex` completion, barrier wait, and state machine read. Validates that current role is `RoleLeader`, `term == expectedTerm`, and `leaderEpoch == expectedEpoch`. Any concurrent stepdown or leadership loss fails closed with `ErrRaftInvalidRoleTransition`, triggering leader redirection rather than serving stale reads from local engine.
    - **Client-Facing GET Routing**: Implemented `ReadRouter` interface in `internal/transport/server.go` and integrated with `ProposalRouter` in `internal/raft/router.go`. In cluster mode, `OpGet` routes through `RouteRead`: leader executes `ReadIndex -> WaitForApplied -> ValidateLeadership -> Engine.Get()`; followers return `StatusNotLeader` with trusted topology leader addresses; candidates and unknown leaders return `StatusNotLeader` without arbitrary client redirects; missing consensus router in cluster mode fails closed (`StatusError`). Standalone mode continues to serve direct unverified `engine.Get()` with zero consensus overhead.
    - **Wire Protocol Preservation**: Preserves wire format (`StatusOk` with value, `StatusKeyNotFound`, `StatusNotLeader` with LeaderID/LeaderAddr, `StatusThrottled`, `StatusServerClosed`, `StatusError`). Preserves `SeqID`. Sanitizes error messages without exposing internal Raft metadata or storage paths.
    - **Comprehensive Verification**: Verified with unit tests (`barrier_test.go`), transport routing tests (`server_read_test.go`), router tests (`router_test.go`), and real 3-node TCP integration tests (`read_barrier_tcp_integration_test.go`) covering lagging state machine hold-and-release, delete linearizability, partitioned leader fail-closed, stepdown during wait, and concurrent readers under race detector.
* **P17-SEC: Phase 17 Security Remediation & Pre-Phase-18 Security Gate** (complete)
  * *Objective*: Remediate all security audit findings on the Phase 17 read-path before Phase 18 chaos testing:
    - **Bounded Concurrent ReadIndex Rounds (Finding A)**: Introduced `MaxActiveReadRounds` (default 512, configurable via `NodeConfig`). Saturated rounds fail fast with `ErrReadIndexThrottled` (mapped to `StatusThrottled` with `"read throttled under load"` on wire), preventing memory exhaustion under reader storms. Proved via saturation, capacity-release, stepdown, and node close tests.
    - **ValidateLeadership Lock Discipline (Finding B)**: Restructured `ValidateLeadership` (and `ReadIndex`) so `Node.mu` is never held across blocking/cross-component calls (`Storage.Term()`). Uses atomic snapshot and revalidation under lock to eliminate TOCTOU race conditions with zero lock inversion.
    - **Corrected Write Semantics (Finding C)**: Audited and corrected all documentation to accurately distinguish local durable append, quorum commit, state machine application, and linearizable read verification. `ProposeWithContext()` acknowledges upon local durable append on leader + valid leadership epoch; it does NOT block on quorum commit. Multi-node linearizable reads (`ReadIndex`) guarantee linearizability over the quorum-committed and applied prefix.
    - **Real Client TCP GET Integration Testing (Finding D)**: Added full TCP wire tests in `client_tcp_read_integration_test.go` through `net.Dial`, `transport.Server`, `ReadRouter`, `ReadIndex`, `WaitForApplied`, `ValidateLeadership`, and `Engine.Get`, verifying standalone GET, cluster leader GET, follower redirect, missing router fail-closed, barrier wait for unapplied entries, delete linearizability, stepdown during wait, and wire `SeqID` preservation.
    - **Test Seam Encapsulation (Finding E)**: Moved safety-sensitive mutation hooks (`SignalAppliedForTest` and `SetReadIndexTestHook`) into `export_test.go` so they are stripped from production builds.
    - **ProposalRouter Immutability (Finding F)**: Removed mutable `ProposalRouter.SetEngine()` to ensure the read path cannot accidentally verify one state machine and read from another.
    - **Physical Partition Assurance**: Added test demonstrating physical TCP peer connection disconnect causes linearizable GETs on isolated leader to fail closed without serving stale data.
    - **Remaining Limitations**: Peer transport authentication (mTLS) remains deferred to Phase 19. Synchronous write quorum acknowledgement and read leases remain deferred. Phase 18 was NOT started. Pre-Phase-18 security gate is COMPLETE.
