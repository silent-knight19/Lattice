...
* **Client Authorization & RBAC**: Zero-trust role-based access control bound to authenticated client TLS certificates via SHA-256 leaf certificate fingerprints (`reader`, `writer`, `admin`). Authorization decisions execute at the transport boundary before storage mutations or Raft consensus proposals, failing closed with uniform permission-denied responses and zero side-effects.
