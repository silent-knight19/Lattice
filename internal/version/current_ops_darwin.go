...
func createTempAt(dirFile *os.File, name string, perm os.FileMode) (*os.File, error) {
	...
	r1, _, errno := syscall.Syscall6(darwinSysOpenat, dirFile.Fd(), uintptr(unsafe.Pointer(nameBytes)), uintptr(flags), uintptr(perm), 0, 0)
	if errno != 0 {
		return nil, errno
	}
...
func renameAt(dirFile *os.File, oldName, newName string) error {
	...
	_, _, errno := syscall.Syscall6(darwinSysRenameat, dirFile.Fd(), uintptr(unsafe.Pointer(oldBytes)), dirFile.Fd(), uintptr(unsafe.Pointer(newBytes)), 0, 0)
	if errno != 0 {
		return errno
	}
	return nil
...
func removeAt(dirFile *os.File, name string) error {
	...
	_, _, errno := syscall.Syscall(darwinSysUnlinkat, dirFile.Fd(), uintptr(unsafe.Pointer(nameBytes)), 0)
	if errno != 0 {
		return errno
	}
	return nil
